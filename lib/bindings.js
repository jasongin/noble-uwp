...
NobleBindings.prototype.notify = function(deviceUuid, serviceUuid, characteristicUuid, notify) {
	...
	this._getCachedCharacteristicAsync(
			deviceUuid, serviceUuid, characteristicUuid).then(characteristic => {
		if (notify) {
			// TODO: Remove this when removing listener.
			setTimeout(() => {}, 10000000);

			characteristic.addListener('valueChanged', e => {
				// TODO: Get buffer data from notification event? Or read it?
				var data = new Buffer();
				debug('notification: ' + deviceUuid + ' ' + serviceUuid + ' ' + characteristicUuid);
				this.emit('notification', deviceUuid, serviceUuid, characteristicUuid, data);
			});
