...
NobleBindings.prototype._onAdvertisementWatcherReceived = function(sender, e) {
	...
	if (manufacturerSections.size > 0) {
		...
		if (manufacturerData.companyId) {
			let companyIdHex = manufacturerData.companyId.toString(16);
			let toAppend = [];
			for (var index = 0; index < companyIdHex.length; index += 2) {
				toAppend.unshift(parseInt('0x' + companyIdHex.slice(index, index + 2), 16));
			}
			deviceRecord.manufacturerData = Buffer.concat(Buffer.from(toAppend),  deviceRecord.manufacturerData);
			debug('    manufacturer data: %s', data.toString('hex'));
		}
	}
