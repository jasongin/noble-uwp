...
NobleBindings.prototype._onAdvertisementWatcherReceived = function(sender, e) {
	...
	if (e.advertisement.dataSections.size > 0) {
		var entry = {};
		for (var i = 0; i < e.advertisement.dataSections.size; i++) {
			var section = e.advertisement.dataSections[i];
			// https://www.bluetooth.com/specifications/assigned-numbers/generic-access-profile
			switch (section.dataType) {
				case 0x03: // complete list of 16-bit service class UUIDs
					var id = Buffer.allocUnsafe(2);
					var buf = rt.toBuffer(section.data);
					id[0] = buf[1];
					id[1] = buf[0];
					entry.uuid = id.toString('hex');
					break;
				case 0x07: // complete list of 128-bit service class UUIDs
					entry.uuid = rt.toBuffer(section.data).toString('hex');
					break;
				case 0x16: // service data
					entry.data = rt.toBuffer(section.data).slice(2);
					break;
				default:
					debug('unhandled data type', section.dataType);
					break;
			}
		}
		if (entry.uuid) {
			deviceRecord.serviceData.push(entry);
		}
		debug('    service data:', deviceRecord.serviceData);
	}
	...
	if (e.advertisementType == BluetoothLEAdvertisementType.scanResponse) {
		let advertisement = {
			...
			serviceData: deviceRecord.serviceData
		};
	...
	return this._getCachedCharacteristicAsync(
		...
		return rt.promisify(characteristic.getDescriptorsAsync, characteristic)(
			...
			descriptor = rt.toArray(result.descriptors)
				.find(d => formatUuid(d.uuid) === descriptorUuid);
			if (!descriptor) {
				throw new Error('Service ' + serviceUuid + ' characteristic ' +
